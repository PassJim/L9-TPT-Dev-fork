...
void Element::Element_SBOMB()
{
	Identifier = "DEFAULT_PT_SBOMB";
	Name = "SBOM";
	Colour = 0x13D5F7_rgb;
	MenuVisible = 1;
	MenuSection = SC_MOD;
	...
	HeatConduct = 29;
	Description = "smaller radius bomb";
		...
		for (auto ry = -1; ry <= 1; ry++)
		{
			if (rx || ry)
			{
				...
				if (!r)
					continue;
				auto rt = TYP(r);
				if (rt!=PT_SBOMB && rt!=PT_BOMB && rt!=PT_EMBR && rt!=PT_DMND && rt!=PT_CLNE && rt!=PT_PCLN && rt!=PT_BCLN && rt!=PT_VIBR && rt!=PT_CRAY && rt!=PT_STOR)
				{
					int rad = 2, nt;
					sim->kill_part(i);
