...
void Element::Element_ROT1()
{
	Identifier = "DEFAULT_PT_ROT1";
	Name = "ROT1";
	Colour = 0x1e1e1e_rgb;
	MenuVisible = 1;
	MenuSection = SC_MOD;
	...
	LowTemperatureTransition = PT_ROT2;
	HighTemperature = 673.0f;
	HighTemperatureTransition = PT_ROT3;

	DefaultProperties.tmp4 = 1000;

	Update = &Element_ROT1_update;
	Graphics = &graphics;
}

int Element_ROT1_update(UPDATE_FUNC_ARGS)
{
	...
	for (auto rx = -1; rx <= 1; rx++)
	{
		for (auto ry = -1; ry <= 1; ry++)
		{
			if (rx || ry)
			{
				...
				if (parts[ID(r)].tmp3 && (TYP(r) == PT_ROT1 || TYP(r) == PT_ROT2 || TYP(r) == PT_ROT3))
				{
				...
				else if (TYP(r) != PT_ROT1 && TYP(r) != PT_ROT2 && TYP(r) != PT_ROT3 && TYP(r) != PT_BRCK && TYP(r) != PT_SAND && TYP(r) != PT_DMND) 
				{
					if (!(rndstore & 0x7))
					{
						...
						if (parts[ID(r)].temp < 305.0f)
							{	parts[i].life +=1;
								if (sim->rng.chance(1,10)&& parts[i].life<15)
								{
									sim->part_change_type(ID(r), x+rx, y+ry, PT_ROT2);
									
									if ((TYP(r) == PT_ROT1 || TYP(r) == PT_ROT2 || TYP(r) == PT_ROT3))
										{
						...
						{	
							...
							if (sim->rng.chance(1,10)&& parts[i].life<15)
							{
								sim->part_change_type(ID(r), x+rx, y+ry, PT_ROT3);
						...
						{
							parts[i].life +=1;
							if (sim->rng.chance(1,10)&& parts[i].life<15) // random infection and won't continue if life <0
							{
								sim->part_change_type(ID(r), x+rx, y+ry, PT_ROT1);
				...
				//protons make ROT1 last forever
